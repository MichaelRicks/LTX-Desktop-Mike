...
class VideoGenerationHandler(StateHandlerBase):
    ...
    def _local_pixels(
        ...
        *,
        ...
        # FORK: 21:9 (~2.37:1) ultrawide is local-only and not modelled in ltx_capabilities.
        # Map it directly, capped at 540p/720p (1080p ultrawide is too heavy). Dims are
        # pre-rounded to /64 so generate rounding doesn't distort the ratio.
        if aspect == "21:9":
            ultrawide = {"540p": (1216, 512), "720p": (1664, 704)}.get(resolution)
